...
const TopicDetail: React.FC = () => {
  ...
  const book = async (e: React.FormEvent) => {
    ...
    if (!topic || !id) return;
    setSubmitting(true);
    await supabase.from('biz_consultancy_bookings').insert({ topic_id: id, ...form });
    fetch('https://famous.ai/api/crm/69ea64be485fe0443f9c974c/subscribe', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: form.email, name: form.full_name, source: 'consultancy-booking', tags: ['consultancy', topic.title] })
    }).catch(() => {});
