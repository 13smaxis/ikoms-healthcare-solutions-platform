...
export async function POST(req) {
  try {
    ...
    const { data: order, error: orderError } = await supabase
      .from('orders')
      .select('orderid, status, totalamount, currency, customerid')
      .eq('orderid', orderId)
      .single();
...
import { validatePaymentInput } from '@/lib/validation';
import crypto from 'crypto';
import { TWOCHECKOUT_CONFIG, is2CheckoutReady } from '@/lib/twocheckout';
...
export async function POST(req) {
  try {
    ...
    // 3. FETCH ORDER FROM IKOMS SCHEMA
    ...
    if (orderError) {
      console.error('[PAYMENT] Order fetch error:', orderError);
      return Response.json(
    ...
    // 5. VERIFY AMOUNT IS VALID
    if (!order.totalamount || order.totalamount <= 0) {
      console.error(`[PAYMENT] Invalid amount for order ${orderId}: ${order.totalamount}`);
      return Response.json(
    ...
    // 6. BUILD 2CHECKOUT CHECKOUT URL
    ...
    console.log(`[PAYMENT] Building 2Checkout URL with amount: ${amount}`);
    ...
    // 7. UPDATE ORDER STATUS TO 'AWAITING_PAYMENT'
    const { error: updateError } = await supabase
      .from('orders')
      .update({ 
    ...
    if (updateError) {
      console.error('[PAYMENT] Failed to update order status:', updateError);
      // Don't fail - still return checkout URL
    } else {
      console.log(`[PAYMENT] ✓ Order ${orderId} status updated to awaiting_payment`);
    }
