    ...
    return (
      <SiteLayout>
        <div className="max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-slate-600 mb-8">Browse our shop to add products, or book a course or consultation.</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/shop/products" className="px-6 py-3 bg-rose-700 text-white rounded-lg font-semibold">Shop products</Link>
            <Link href="/training/courses" className="px-6 py-3 border border-slate-300 text-slate-700 rounded-lg font-semibold">Browse courses</Link>
          </div>
        </div>
      </SiteLayout>
    );
  ...
  return (
    <SiteLayout>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <p className="text-slate-600 mb-8">{cart.reduce((s, i) => s + i.quantity, 0)} item(s)</p>
            ...
            {cart.map((item, i) => (
              <div key={i} className="bg-white border border-slate-200 rounded-xl p-4 flex gap-4">
                ...
                <div className="text-right">
                  <div className="font-bold text-slate-900">{fmt(item.price * item.quantity)}</div>
